...
class LogoutApi {
  ...
  Future<void> logout() async {
    try {
      await _dio.post('/api/accounts/v1/logout/');
    } on DioException catch (e) {
      throw _handleError(e);
    } finally {
      // Always clear all auth data from local storage after logout attempt
      await _clearAllAuthData();
    }
  }

  /// Clear all auth data from local storage (both user and session)
  Future<void> _clearAllAuthData() async {
    try {
      // Clear user data box
      try {
        final userBox = await Hive.openBox<Map<dynamic, dynamic>>('auth_users');
        await userBox.delete('current_user');
        debugPrint('Cleared current_user from auth_users box');
      } catch (e) {
        debugPrint('Error clearing user data: $e');
      }

      // Clear session data box
      try {
        final sessionBox = await Hive.openBox<Map<dynamic, dynamic>>('auth_sessions');
        await sessionBox.delete('current_session');
        debugPrint('Cleared current_session from auth_sessions box');
      } catch (e) {
